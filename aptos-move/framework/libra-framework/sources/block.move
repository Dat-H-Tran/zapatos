...
module aptos_framework::block {
    ...
    fun block_prologue(
        ...
        assert!(
            proposer == @vm_reserved || validator::is_current_epoch_validator(proposer),
            error::permission_denied(EINVALID_PROPOSER),
        );

        let proposer_index = option::none();
        if (proposer != @vm_reserved) {
            proposer_index = option::some(validator::get_validator_index(proposer));
        };
        ...
        validator::update_performance_statistics(proposer_index, failed_proposer_indices);
